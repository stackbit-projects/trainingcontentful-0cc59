              ...
              <div id="docs-nav-inside" className="docs-nav-inside sticky">
                ...
                <div className="docs-nav-menu">
                  <ul id="docs-menu" className="docs-menu">
                    <li className={classNames('docs-menu-item', {'current': _.get(page, 'stackbit_url_path', null) === _.get(root_page, 'stackbit_url_path', null)})}>
                      <Link href={withPrefix(_.get(root_page, 'stackbit_url_path', null))}>{_.get(root_page, 'title', null)}</Link>
                    </li>
                    {_.map(_.get(site, 'data.doc_sections.sections', null), (section, section_idx) => {
                        ...
                        let child_pages = _.orderBy(getPages(this.props.pages, section_path), 'weight');
                        let child_count = _.size(child_pages);
                        let has_children = (child_count > 0) ? (true) : false;
                        let is_current_page = (_.get(page, 'stackbit_url_path', null) === _.get(section_page, 'stackbit_url_path', null)) ? (true) : false;
                        let is_active = _.get(page, 'stackbit_url_path', null).startsWith(_.get(section_page, 'stackbit_url_path', null));
                        return (<React.Fragment key={section_idx + '.1'}>
                          <li key={section_idx} className={classNames('docs-menu-item', {'has-children': has_children, 'current': is_current_page, 'active': is_active})}>
                            <Link href={withPrefix(_.get(section_page, 'stackbit_url_path', null))}>{_.get(section_page, 'title', null)}</Link>
